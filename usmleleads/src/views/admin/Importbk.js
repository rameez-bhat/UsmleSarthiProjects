...
const downloadExcelTemplate = () => {
  ...
  const templateData = [
    ["Contact Source","Service", "Webinar Name", "Webinar Date", "First Name", "Last Name", "Email Address","Country Code", "Phone Without Country Code","Is Sarthi Student?","Date of Enquiry","Interested In","Lead Owner", "Which Match Season are you planning for?", "YOG", "Step 1", "Step 2", "Step 3", "Specialty", "Name of Medical School", "Country of Medical School","Lead Notes","Service Status","Planned Start Date","Rotation Pushed","Followup Rotation Pushed","Next Follow-up Date", "Question1", "Answer1", "Question2", "Answer2", "Question3", "Answer3", "Question4", "Answer4"], // Headers
    ["webinar/workshop","Match", "Financial planning for 2026 Match", "05/25/2024", "WImport1", "Test1", "wit1@gmail.com","+91", "9090909090","yes","05/25/2024","rotation","shitanshu@usmlesarthi.com","Match 2026 (Sept 2025)", "2024", "Pass", "Not Taken", "Not Taken", "Internal Medicine", "Navodaya Medical College (Raichur, India)", "India","This is Lead Note","enrolled","05/25/2024","L1IMMIT","L1IMMIT","05/25/2024", "Question1", "Answer1", "What are you here for", "i am here for rotation", "Question3", "Answer3", "Question4", "Answer4"], // Sample Row
  ];
...
try
{
  ...
  const leadDataServices = {
    ...
    {
      leadDataServices.contactsourceseventname=row["Webinar Name"];
      if (row["Webinar Date"] && String(row["Webinar Date"]).trim() !== "")
      {
        leadDataServices.contactsourceseventdate=firestoreQueries.Timestamp.fromDate(
        convertExcelDateString(row["Webinar Date"]));
      }
    }
    if (row["Webinar Date"] && String(row["Webinar Date"]).trim() !== "")
    {
      leadDataServices.contactsourcespecialtywebinarworkshopdate=firestoreQueries.Timestamp.fromDate(
      convertExcelDateString(row["Webinar Date"]));
    }
    ...
    {
      leadDataServices.nextfollowupdate=firestoreQueries.Timestamp.fromDate(convertExcelDateString(row["Next Follow-up Date"]));
      leadDataServices.followupsrequired="yes";
    }
    if(row["Service Status"].trim()!="")
    {
       row["Service Status"]=row["Service Status"].toLowerCase();
       leadDataServices.servicestatus=row["Service Status"];
    }
    if(row['Contact Source'].trim()!=="")
    {
      ...
      if(row?.['Specialty'])
      {
        leadDataServices.contactsourcespecialty=row?.['Specialty'];
      }
 ...
 if (existingLeads.length) {
   ...
   // ExistingLeadsData.matchapplicationsession=leadData.matchapplicationsession;
    ...
    if (services['data'].length)
    {
      ...
      if(!ExistingServiceData?.nextfollowupdate && leadDataServices?.nextfollowupdate)
      {
        ShouldupdateService=true;
        ExistingServiceData.nextfollowupdate=leadDataServices.nextfollowupdate;
        ExistingServiceData.followupsrequired="yes";
      }
 ...
 RotationList.map((item) => {
  ...
  showLoading();
    ...
    reader.onload = async (event) => {
      ...
      const jsonData = XLSX.utils.sheet_to_json(sheet, {
        header: 1, // Get all rows as arrays with the first row as headers
      });
