...
export default function CoreUIFirestoreTable({ collectionName="Housings", pageSize = 100 }) {
    ...
    or(
      where("title", ">=", text)
        ...
        if (targetPageIndex === 0) {
          q = query(colRef, searchConditions, orderBy('title'), limit(pageSize + 1))
        } else {
          const cursor = knownCursors[targetPageIndex - 1]
          q = query(colRef, searchConditions, orderBy('title'), startAfter(cursor), limit(pageSize + 1))
        }
      } else {
        // Default query (no search)
        searchConditions = where("city", "!=", "Type");
        if (targetPageIndex === 0) {
          q = query(colRef,searchConditions, orderBy('title'), limit(pageSize + 1))
        } else {
          const cursor = knownCursors[targetPageIndex - 1]
  ...
  const confirmDelete = window.confirm(
    `Are you sure you want to delete housing ${item.title || item.housingId}`
  );
  ...
  try {
    ...
    // Delete from UsersRoles
  	await DeleteDocumentWhere("Housings","housingId","==", item.housingId);
  ...
  function handleEdit(row) {
    ...
    navigate(`/admin/edithousing/${row.housingId}`)
  }
      ...
      <CTable hover>
        <CTableHead>
          <CTableRow>
            ...
            <CTableHeaderCell scope="col">Housing Name</CTableHeaderCell>
          ...
          {loading ? (
            ...
            rows.map((row, idx) => (
              <CTableRow key={row.id}>
                ...
                <CTableDataCell>{row.state || '-'}</CTableDataCell>
                <CTableDataCell>{row.city || '-'}</CTableDataCell>
                <CTableDataCell>{row.zipCode || '-'}</CTableDataCell>
               <CTableDataCell>
                  <Button
    size="small"
    color="success"
