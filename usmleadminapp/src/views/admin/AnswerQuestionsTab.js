...
const fetchSeenByNames = async (msgId, readByUids) => {
  ...
  let whatsappNumber= userData?.WhatsappCountry?.phoneCode+userData?.WhatsappNumber
    e.preventDefault();
    ...
    if(typeof whatsappNumber=="string" && whatsappNumber=="undefined")
    {
    ...
    if(WhatsAppResponse.status=="success")
    {
    	...
    	//await addDoc(collection(db, 'UserCommonServiceNotes',WhatsAppResponse.messageid), DataToBeAdded);
