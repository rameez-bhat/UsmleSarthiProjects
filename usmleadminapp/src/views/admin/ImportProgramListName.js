...
if (row?.['NRMP Number (Preliminary)']) {
 ...
 let WhereOrObject=[];
    				  WhereOrObject=[{"name":"Frieda","condition":"==","value":Number(HospitalProgramInfo['Frieda'])},{"name":"PId","condition":"==","value":PId}];
    				...
    				if(resultsInfo.status==="success")
    				{
    					if(resultsInfo.data.length)
    					{
    						...
    						{
    							let HId=hospitalInfo?.HId;
    							const HospitalTableData = await FetchDataFromCollection("Hospital", 20, "HId", "==", HId, 0);
    				...
    				//if(NotFound)
    				{
    					  WhereOrObject=[{"name":"Frieda","condition":"==","value":String(HospitalProgramInfo['Frieda'] ?? '')},{"name":"PId","condition":"==","value":PId}];
    		  				//console.log("WhereOrObject===>",WhereOrObject)
    					const resultsInfoS = await SelectWithWhereAnd("HospitalProgramInfo", WhereOrObject);
    					if(resultsInfoS.status==="success")
    					{
    						if(resultsInfoS.data.length)
    						{
    							for (const hospitalInfo of resultsInfoS.data) 
    							{
    								let HId=hospitalInfo?.HId;
    								const HospitalTableData = await FetchDataFromCollection("Hospital", 20, "HId", "==", HId, 0);
    								for (const hospital of HospitalTableData) 
