  ...
  const handleReset = async () => {
    ...
    try {
      ...
      console.log("error--->",error)
        ...
        url: `${currentDomain}/reset-password?email=${encodeURIComponent(email)}&customData=someValue`,
        handleCodeInApp: true, // Ensures the link opens in the app
      };
		console.log("handleCodeInApp---->",actionCodeSettings)
      // ✅ If email exists, send reset email
      let ret=await sendPasswordResetEmail(auth, email,actionCodeSettings);
      console.log("ret--->",ret)
