import { ALGORITHM_VERSION, answerScores, componentMap, thresholds } from './config';
...
export function evaluateEligibility(answers:any, override:any = null) {
  // Backward compatibility for assessments saved before E3 was split.
  const normalized = { ...answers };
  if (!normalized.E3A && normalized.E3) normalized.E3A = normalized.E3;
  if (!normalized.E3B && normalized.E3) normalized.E3B = normalized.E3;
  const required = ['E1','E2','E3A','E3B','E4'];
  if (required.some(q => !normalized[q])) return {status:'INCOMPLETE',blocking:true,warnings:['Complete all eligibility questions.']};
  if (required.some(q => normalized[q] === 'NOT_VERIFIED')) {
    const unresolved = required.filter(q => normalized[q] === 'NOT_VERIFIED');
    return {status:'RESEARCH_REQUIRED',blocking:true,unresolved,warnings:unresolved.map(q => `${q} has not been confirmed.`)};
  }
  const mismatches = required.filter(q => normalized[q] === 'NOT_ALIGNED');
  if (mismatches.length) {
    const hasOverride = normalized.E5 && normalized.E5 !== 'NO_OVERRIDE' && override && override.note && String(override.note).trim().length > 0 && override.date;
    if (!hasOverride) return {status:'NOT_ALIGNED',blocking:true,mismatches,warnings:['Not eligible based on reported criteria. A documented override is required to continue scoring.']};
    return {status:'OVERRIDDEN',blocking:false,mismatches,warnings:['Eligibility was overridden; the original mismatch remains part of the record.'],override};
  }
  const unavailable = required.filter(q => normalized[q] === 'SOURCE_NA');
  return {status:'ALIGNED',blocking:false,unavailable,warnings:unavailable.length ? ['One or more eligibility requirements were not available from reliable sources; confidence may be reduced.'] : []};
}
...
export function calculateProgram(input:any) {
  ...
  confidenceMissing += Number(eligibility.unavailable?.length || 0);
