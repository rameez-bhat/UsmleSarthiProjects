...
export class ResearchService {
  ...
  async getAllHospitals() {
   ...
   async getAllMatchPlans() {
    return this.firestore.collection("MatchPlans", ref => ref.where("Type", "==", "Research").where("ActiveInActive", "!=", "inactive").orderBy("ActiveInActive", "asc").orderBy("rank", "asc")).get().toPromise().then(snap => {
      const result = {};
      snap.forEach(doc => {
  ...
  async getMatchPlanByPid(Pid: string) {
    return this.firestore.collection("MatchPlans", ref => ref.where("Pid", "==", Pid))
      .get()
      .toPromise()
