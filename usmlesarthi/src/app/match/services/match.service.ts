...
export class MatchService {
  ...
  async getAllHospitals() {
   ...
   async getAllMatchPlans() {
    return this.firestore.collection("MatchPlans", ref => ref.where("Type", "==", "Match").where("ActiveInActive", "!=", "inactive").orderBy("ActiveInActive", "asc").orderBy("rank", "asc")).get().toPromise().then(snap => {
      const result = {};
      snap.forEach(doc => {
