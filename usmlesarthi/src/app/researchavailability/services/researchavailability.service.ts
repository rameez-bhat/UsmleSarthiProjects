...
export class ResearchAvailabilityService {
  matchlist: any = [];
  hospitalsFetched: boolean = false;

  constructor(private firestore: AngularFirestore) { }
  async getSelectedMatch(locationCode) {
    this.matchlist = {};
    let docsRef = await this.firestore.collection("MatchPlans", ref => ref.where("Pid", "==", locationCode)).get().toPromise();
    for (let doc of docsRef.docs){
      let data = doc.data();
      data.id = doc.id;
      data.fee = typeof (data.fee) === "number" || (typeof (data.fee) === "string" && !data.fee.includes("$")) ?  Number(data.fee) : Number(data.fee);
